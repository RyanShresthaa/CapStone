...
import { trekAPI, userAPI } from '../services/api';
import { calculateTrekScore, buildDiverseRecommendations } from '../utils/trekRecommenderLogic';
...
function normalizeTrekFromApi(row) {
  ...
  return {
    ...
    id: row.id,
    image,
    altitude: typeof alt === 'number' ? alt : Number(alt) || 0,
    minExperience: row.minExperience || 'Beginner',
    highlights: row.highlights || [],
    itinerary: row.itinerary || [],
    reviews: row.reviews || [],
    accommodation: row.accommodation || [],
    sustainabilityScore: row.sustainabilityScore ?? 5,
    bestSeason: row.bestSeason || 'Spring/Autumn',
    groupSize: row.groupSize || '2–12 people',
  };
}
...
const TrekRecommender = () => {
  ...
  const [trekDatabase, setTrekDatabase] = useState(TREK_FALLBACK_DATABASE);
  const [treksLoading, setTreksLoading] = useState(true);
  const [treksError, setTreksError] = useState(null);
  ...
  const budgetClass = (b) =>
    ({ Low: 'low', Medium: 'medium', High: 'high' }[b] || 'medium');
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setTreksError(null);
      try {
        const { data } = await trekAPI.getAllTreks();
        const rows = Array.isArray(data) ? data : [];
        const normalized = rows.map(normalizeTrekFromApi).filter(Boolean);
        if (!cancelled && normalized.length) {
          setTrekDatabase(normalized);
        } else if (!cancelled && !normalized.length) {
          setTreksError('empty');
        }
      } catch {
        if (!cancelled) {
          setTreksError('failed');
          toast.error('Could not load treks from server — using offline list.');
        }
      } finally {
        if (!cancelled) setTreksLoading(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const refreshWishlistFromServer = useCallback(async () => {
    ...
    try {
      ...
      const rows = Array.isArray(data) ? data : [];
      const treks = rows
        .map((w) => (w.trek ? normalizeTrekFromApi(w.trek) : null))
        .filter(Boolean);
      setWishlist(treks);
    } catch {
      toast.error('Could not sync wishlist from your account.');
    }
  }, [isAuthenticated]);
  ...
  const handleAddToWishlist = async (trek) => {
    ...
    if (isInWishlist) {
      if (isAuthenticated) {
        try {
          await userAPI.removeFromWishlist(trek.id);
        } catch {
          toast.error('Could not remove from server wishlist.');
          return;
        }
      }
      setWishlist((list) => list.filter((item) => item.id !== trek.id));
      toast.success(`${trek.name} removed from wishlist!`);
    } else {
      if (isAuthenticated) {
        try {
          await userAPI.addToWishlist(trek.id);
        } catch (err) {
        ...
        {showWishlist ? (
              ...
              {wishlist.map((trek) => (
                <div key={trek.id} className="trek-card wishlist-card">
                  ...
                  <div className="trek-card-content">
                    ...
                    <div className="trek-stats-grid" role="list">
                      <div className="trek-stat-tile" role="listitem">
                        <FaClock className="trek-stat-icon" aria-hidden />
                        <span className="trek-stat-label">Duration</span>
                        <span className="trek-stat-value">{trek.duration}</span>
                      </div>
                      <div className="trek-stat-tile" role="listitem">
                        <FaMountain className="trek-stat-icon" aria-hidden />
                        <span className="trek-stat-label">High point</span>
                        <span className="trek-stat-value">{typeof trek.altitude === 'number' ? `${trek.altitude.toLocaleString()} m` : trek.altitude}</span>
                      </div>
                      <div className="trek-stat-tile" role="listitem">
                        <FaUsers className="trek-stat-icon" aria-hidden />
                        <span className="trek-stat-label">Group size</span>
                        <span className="trek-stat-value">{trek.groupSize}</span>
                      </div>
                      <div className="trek-stat-tile" role="listitem">
          ...
          <div className="recommender-form-container">
            ...
            {!treksLoading && treksError ? (
              <p className="trek-recommender-banner trek-recommender-banner--warn" role="status">
                Showing offline trek list (server unavailable or empty).{' '}
                  ...
                  <div className="trek-card-content">
                    ...
                    <div className="trek-stats-grid" role="list">
                      ...
                      <div className="trek-stat-tile" role="listitem">
                        <FaMountain className="trek-stat-icon" aria-hidden />
                        <span className="trek-stat-label">High point</span>
                        <span className="trek-stat-value">
                          {typeof trek.altitude === 'number' ? `${trek.altitude.toLocaleString()} m` : trek.altitude}
                        </span>
                      </div>
                      <div className="trek-stat-tile" role="listitem">
                        <FaUsers className="trek-stat-icon" aria-hidden />
                    ...
                    <div className="trek-sustainability">
                      ...
                      <div className="trek-sustainability-text">
                        <span className="trek-sustainability-label">Sustainability</span>
                        <span className="trek-sustainability-score">{trek.sustainabilityScore}/10</span>
                      </div>
                    </div>

                    <div className="trek-accommodation">
                      <h4 className="trek-section-heading">Where you&apos;ll stay</h4>
                      <div className="accommodation-options">
                        {trek.accommodation.map((option, idx) => (
                          <div key={idx} className="accommodation-option">
                            <div className="accommodation-option-inner">
                              <div className="accommodation-type">{option.type}</div>
                              <p className="accommodation-description">{option.description}</p>
