...
require('dotenv').config({ path: path.join(__dirname, '.env') });    res.status(500).json({ message: 'Server error' });
  }
});
...
app.post(
  ...
  try {
    ...
    const user = await User.findOne({ email: email.toLowerCase() });    if (!user) return res.status(401).json({ message: 'Invalid credentials' });
...
app.get('/api/auth/verify-reset-token/:token', async (req, res) => {
  try {
    ...
    res.json({ valid: true, email: user.email });  } catch (err) {
    res.status(500).json({ message: 'Server error' });
  }
});
